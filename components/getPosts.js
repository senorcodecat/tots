...
module.exports = function(webserver, db) {
    ...
    webserver.get('/posts/feed/live', function(req, res) {
        ...
        db.follow.find({
            ...
            var following = following.map(function(f) {
                return f.following;
            });
            ...
            db.posts.find({
                ...
                date: -1
            }).limit(limit).skip(skip).exec(function(err, posts) {
    ...
    webserver.get('/get/invite', function(req, res) {

        // get user for whom this is supposedly an invite
        db.users.findOne({username: req.query.username},function(err, profile) {
            if (profile) {
                db.invites.findOne({code: req.query.code, user: profile._id, valid: true},function(err, invite) {
                    if (invite) {
                        res.json({
                            ok: true,
                            data: profile,
    ...
    webserver.get('/me/invite', function(req, res) {
        if (req.user_profile) {
            db.invites.find({
                user: req.user_profile._id,
                valid: true
    ...
    webserver.post('/me/invite', function(req, res) {
        if (req.user_profile) {
            ...
            db.invites.update({
                ...
                // create a new invite code for this user
                var invite = new db.invites();
                invite.user = req.user_profile._id;
                invite.code = db.generateCode();
    ...
    webserver.get('/me/invitees', function(req, res) {
        if (req.user_profile) {
            db.users.find({
    ...
    webserver.get('/user/followers', function(req, res) {
        ...
        db.users.findOne({
            ...
            }).populate('user').exec(function(err, following) {

                var users = following.map(function(f) {
                    return f.user;
                });
                res.json({
                    ok: true,
    ...
    webserver.get('/user/following', function(req, res) {
        ...
        db.users.findOne({
            ...
            }).populate('following').exec(function(err, following) {
        ...
        db.users.findOne({
            ...
            checkFollowing(req.user_profile, user_profile, function(following, followback) {
                db.faves.find({
                    ...
                    date: -1
                }).limit(limit).skip(skip).exec(function(err, faves) {
                    var posts = [];
    ...
    function checkFollowing(current_user, profile_user, cb) {
        ...
        if (!current_user) {
            ...
            db.follow.findOne({
                ...
                following: profile_user._id
            }, function(err, following) {
                db.follow.findOne({
                    user: profile_user._id,
                    following: current_user._id
                }, function(err, followback) {
                    if (following) {
                        following = true;
                    } else {
                        following = false;
